...
url = 'http://club.jd.com/comment/productCommentSummaries.action?referenceIds={}&callback=jQuery9779506&_={}' \
            .format(id, str(int(time.time() * 1000)))
print(url)
text = requests.get(url).text
s = re.findall('{"CommentsCount.+}', text)[0]
commentData = json.loads(s)
print(commentData)
commentCount = commentData['CommentsCount'][0].get('CommentCountStr')#评价总数
goodComment =  commentData['CommentsCount'][0].get('GoodCountStr')#好评数
generalComment = commentData['CommentsCount'][0].get('GeneralCountStr')  #中评数
poolComment =  commentData['CommentsCount'][0].get('PoorCountStr') #差评数
print(commentCount,goodComment,generalComment,poolComment)
'''function getUid() {
    var a = readCookie("__jda");
    var b = "";
    return a && a.indexOf(".") > -1 && (b = a.split(".")[1]),
    b
}'''
#     data_now['prices'] = []
#     data_now['urls'] = []
